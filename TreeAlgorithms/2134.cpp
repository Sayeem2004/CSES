...
int main() {
    ...
    heavy_decomp<long long> HLD(N, ADJ, VAL, [](long long a, long long b) { return std::max(a, b); });
    for (int i = 0; i < Q; i++) {
        long long t, a, b; std::cin >> t >> a >> b;
        if (t == 1) HLD.update(HLD.POS[a-1], b);
        else std::cout << HLD.calc(a-1, b-1) << " ";
    }
    std::cout << "\n";
}
